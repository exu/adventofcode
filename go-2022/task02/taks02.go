...
func Result(opponent, you Shape) int {
	if isWin(opponent, you) {
		return WinScore + ShapeScore[you]
	} else if opponent == you {
		return DrawScore + ShapeScore[you]
	} else {
		return LostScore + ShapeScore[you]
	}
}

func isWin(opponent, you Shape) bool {
	for action, reaction := range WinMap {
		if opponent == action && you == reaction {
			return true
		}
	}
	return false
...
func Split(in string) (out [][2]Shape) {
	spl := strings.Split(in, "\n")
	for _, e := range spl {
		s := strings.Split(e, " ")
		out = append(out, [2]Shape{
			ShapeMap[s[0]],
			ShapeMap[s[1]],
		})
